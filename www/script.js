...
function RegistrarUbicacion(){			
	var subic=prompt("Escriba su dirección breve: Calle, No. y Colonia");
	if(navigator.geolocation){
		navigator.geolocation.getCurrentPosition(function(position){
			document.getElementById("domicilio-geo").innerHTML= "Ver mapa("+position.coords.latitude + "," + position.coords.longitude+")<br/>" + subic;
			document.getElementById("domicilio-geo").setAttribute("href","javascript:window.open('https://www.google.com/maps/?q=" + position.coords.latitude + "," + position.coords.longitude+"');");
			$.post(url + "/logic/controlador.aspx?op=RegistrarUbicacion&seccion=domicilios",{latitud:position.coords.latitude,longitud:position.coords.longitude,ubicacion:subic},function(xmlDoc){
				if(GetValor(xmlDoc,"estatus")=="1"){
					PonerDomicilioEntrega(GetValor(xmlDoc,"domicilio"));
				}
			});	
		}, function(error){alert(error.code);}, { enableHighAccuracy: true });
	}else{
...
function PonerDomicilioEntrega(domicilio){
	$(".h-dom").each(function(indice){this.innerHTML=domicilio});
}
	
function IsPhoneGap(dato) {
    isPhonegapApp = dato;
}
...
$(document).ajaxError(function (event, jqxhr, settings, thrownError) {
    if (!window.navigator.onLine) {
        window.top.postMessage({ funcion: 'Conectar', datos: [null] }, '*');
    }
});
...
window.addEventListener("message", function (event) {
	try {
		switch(event.data.funcion){
			case "IsPhoneGap":IsPhoneGap(true);break;
			case "ActivarAlarma_":ActivarAlarma_(event.data.params[0]);break;
			case "PantallaMostrar":PantallaMostrar(event.data.params[0],event.data.params[1]);break;
			case "CargarImagenPhoneGap":CargarImagenPhoneGap(event.data.params[0]);break;				
		}
	} catch (e) { alert(e.message); }
}, false);

window.onload = function () {
    window.top.postMessage({ funcion: 'WebViewLista' }, '*');
    IniciarApp();
}
...
function IniciarApp() {
	...
	/*$("#buscador-prodserv").scroll(function(){
		if(document.getElementById("buscador-prodserv").scrollTop>(parseInt(document.getElementById("buscar-prodserv").offsetTop,10) + parseInt(document.getElementById("btn-buscar-ps").offsetTop,10))){						
			document.getElementById("go_up").style="display:block;";
		}else{
			document.getElementById("go_up").style="display:none;";			
			document.getElementById("btn-buscar-ps").focus();
		}
	});*/
    InicializarApp();
    domicilios_reg = [];
...
function EstablecerDimensiones() {
    ...
    styleStr += ".menu button img {height:" + (heightApp - 100) / 4 + "px !important;overflow-y:auto;}";
    ...
    styleStr += ".pantalla-4 {height:" + (heightApp) + "px !important;overflow-y:auto;}";
    ...
    styleStr += ".menu li {height:" + (heightApp - 50) / 3 + "px !important;margin-bottom:" + (heightApp - 50) / 10 + "px !important;}";
...
function EstablecerLogo() {
    try {
        var ca = window.localStorage.getItem("codigoActivacion");
    } catch (e) { alert(e.message); }
    
    var imgs = $("img.logo");
    for (var i = 0; i < imgs.length; i++) {
        imgs[i].setAttribute("src", "img/logo.png?v=1.0");
    }
}
...
function RegistrarVariables(datos, xmlDoc) {
    ...
    window.localStorage.setItem("email_", GetValor(xmlDoc, "correo"));
    window.localStorage.setItem("contrasena_", GetValor(xmlDoc, "contrasena"));
	...
	var domicilio=innerHTML = GetValor(xmlDoc, "domicilio_u");
    $(".h-dom").each(function(){this.innerHTML=domicilio});
    ...
    try {
        if (window.localStorage.getItem("email_")) {
            $.post(url + 'logic/controlador.aspx' + '?op=ObtenerFuncionesHabilitadas&seccion=seguridad&funciones=6', function (xmlDoc) {
                ...
                topicos_suscribir.push('FRA_1_' + window.localStorage.getItem("codigoActivacion"));
                //RegistrarDomicilios   
                domicilios_reg = window.localStorage.getItem("domicilios").split(",");
                for (var i = 0; i < domicilios_reg.length; i++) {
                    topicos_suscribir.push('FRA_1_' + window.localStorage.getItem("codigoActivacion") + "_dom_" + domicilios_reg[i]);
                }
                //Registrarfunciones			
                _func_hab_ = [];
                ...
                for (var n = 0; n < funcionesRecibidas.length; n++) {
                    ...
                    topicos_suscribir.push('FRA_1_' + window.localStorage.getItem("codigoActivacion") + "_fun_" + _func_hab_[n]);
                }
                window.top.postMessage({ funcion: 'SuscribirNotificaciones', datos: topicos_suscribir }, '*');
            });
        }
    } catch (e) { }
}
...
function ActivarAlarma() {
    if (confirm("Confirme que desea activar la Alarma Vecinal.")) {
...
function ActivarTimbre_(contenidovoz) {
    _root.cordova.plugins.notification.local.schedule({
        id: 1,
        title: "SAFRA",
        message: "Tiene Visita"
    });

    _root.cordova.plugins.notification.local.on("click", function (notification) {
        PresentarVisita();
    });
    ...
    switch (catalogo) {
        ...
        case "solicitudes":
            ...
            btn.onclick = function () {
                ...
                ; break;
        case "prodserv":
			document.getElementById("check-negocio").style.display = (GetValor(xmlDoc, "esmismo")=="1" ? "block" : "none");				
            var pantalla = document.getElementById("detalle-" + catalogo);
            pantalla.setAttribute("clave", clave);
            var btn = document.getElementById("activar-negocio");
            btn.checked = (GetValor(xmlDoc, "activo") == "true");
            btn.value = clave;
            document.getElementById("autorizar-negocio").checked = (GetValor(xmlDoc, "moderado") == "true");
            cont =
...
function AbrirDocumento(url, target, extension) {
    ...
    if (!isPhonegapApp) {
        ...
        window.top.postMessage({ funcion: 'downloadFile', datos: {url:url, nombreArch:nombreArch, extension:extension, email:window.localStorage.getItem("email_"), contrasena:window.localStorage.getItem("contrasena_"), codigoActivacion:window.localStorage.getItem("codigoActivacion")} }, '*');
    }
}
...
function PintarItemEditar(catalogo, clave, xmlDoc0) {
    ...
    switch (catalogo) {
        ...
        case "regen_egrepro":
            ...
            }else if(catalogo == "regen_tiposgastos"){
				CargarDatosFrmMap(xmlDoc, { clave: 'clave-regen_tiposgastos', concepto: 'r_tg_concepto', fecha_pago: 'r_tg_fecha_pago',importe:'r_tg_importe',descripcion:'r_tg_descripcion',indicereferencia:'in-regen_tiposgastos'});
        ...
        case "directorio":
            ...
            GetValor(xmlDoc, "privado")=="true"?document.getElementsById("privado").setAttribute("checked"):document.getElementsById("privado").removeAttribute("checked");
            ; break;
        case "encuestas":
...
function AgregarPeriodo(){	
	if(confirm("Confirme que desea agregar el período de Informe")){
		var fecha1 = document.getElementById("i-fecha1").value;
		var fecha2 = document.getElementById("i-fecha2").value;
		$.post(url + 'logic/controlador.aspx' + '?op=AgregarInformePeriodo&seccion=transparencia', { fecha1: fecha1,fecha2: fecha2}, function (xmlDoc1) {
...
function IniciarEditar(esNuevo, catalogo, solotexto, intercambio, clave, callback) {
    ...
    if (esNuevo) {
        ...
        $.post(url + 'logic/controlador.aspx' + '?op=ObtenerItem&seccion=' + catalogo + '&catalogo=' + catalogo, datos, function (xmlDoc) {
            //QuitarEspera();
            if (callback) {
                callback(xmlDoc);
            } else {
...
function MostrarAgregarProdServ(){
	document.getElementById("lista-cat-prodserv").style.display="block";
}

function ConvertirEnItemTicket(obj){
	var ctd=parseInt(document.getElementById("ctd").value,10);
	var concepto=$(obj.sel).find("span.concepto")[0].innerHTML;
	var importe=$(obj.sel).find("span.importe")[0].getAttribute("importe");
	obj.sel.innerHTML='<span class="conteo" conteo="' + ctd + '" style="font-size:1em; width:8%;float:left;">' + ctd + '</span>'+
		'<span class="t-1 concepto" style="float: left;width:45%;clear:none;font-weight:bold;font-size:0.7em;padding-top:0px;">'+ concepto +'</span>'+
		'<span class="t-3 ctd_imp" style="float:right;font-weight:bold;font-size:0.7em;width:20%;text-align:right;" ctd_imp="' + ctd*parseFloat(importe) + '">'+ MoneyFormat(ctd*parseFloat(importe)) + '</span>'+
		'<span class="t-3 importe" style="float:right;font-weight:bold;font-size:0.7em;width:20%;text-align:right;" importe="' + importe + '">'+ MoneyFormat(importe) + '</span><hr class="clearn">';		
	return obj.sel;
}

function ResolverKey(ev){
...
function RegistrarPedido(){
	var productos=[];
	$("#l-pedido li.producto").each(function(indice){
		productos.push($(this).find("span.conteo")[0].getAttribute("conteo")+"-"+this.getAttribute("clave"));
	});
	var total=0.00;
	$("#l-pedido li.producto").find("span.ctd_imp").each(function(indice) {
        total += parseFloat(this.getAttribute("ctd_imp"));
    });
	var negocio=document.getElementById("l-pedido").getAttribute("negocio");
	$.post(url + "logic/controlador.aspx?op=Guardar&seccion=pedidos",{productos:productos.join(","),negocio:negocio,total:total},function(xmlDoc){
		alert(GetValor('mensaje'));		
	});
}

function BuscarProductos(obj){
	var obj=document.getElementById("b-prod");
	var buscar=obj.value;
	var excluir=[];
	$("#l-pedido li.producto").each(function(indice){
		excluir.push(this.getAttribute("clave"));
	});
	CargarCatalogo('regegresos.cargar_productos', function(){
		CambioPantalla('lista-vigilancia', 'buscador-prodserv');
		obj.blur();
	},{negocio:1, buscar:buscar, excluye:excluir.join(",")},'lista-productos',ObtenerUnNegocio,'categoria');
}

function MostrarAnimacion(_img_s){
	window._img=document.createElement("img");
	window._span_cc=document.getElementById("cuenta-car");
	window._span_cc.style.transform="scale(2,3)";
	window._span_cc.style.color="red";
	window._img.src=_img_s;
	window._img.className="temporal-in";
	window._img.className="temporal-in temporal-p";
	document.body.appendChild(_img);
	window.setTimeout(function(){
		window._img.className="temporal-in temporal-out";
		window._span_cc.style.transform="scale(1,1)";
		window._span_cc.style.color="#2c5878";
		document.body.removeChild(window._img);
	},500);
}

function AgregarAlPedido(){	
	var vent_sel=document.getElementById('desc-prod');
	if(parseInt(document.getElementById("ctd").value,10)>0){
		document.getElementById("notif").play();
		MostrarAnimacion("img/addcar.gif");
		var ul_parent=vent_sel.sel.parentNode;
		document.getElementById("l-pedido").appendChild(ConvertirEnItemTicket(vent_sel));
		if(ul_parent.getElementsByTagName("li").length==0){
			ul_parent.parentNode.parentNode.removeChild(ul_parent.parentNode);
		}		
	}else{
		try{
			document.getElementById("l-pedido").removeChild(vent_sel.sel);			
			document.getElementById("basura").play();
			MostrarAnimacion("img/quitcar.gif");
		}catch(e){}		
	}
	window.localStorage.setItem('neg_' + vent_sel.parentIndice,document.getElementById("l-pedido").innerHTML);
	vent_sel.style.display='none';	
	RealizarConteoCuenta();
	BuscarProductos();
}

function ObtenerUnNegocio(catalogo,item){
	var itemli = document.createElement("li");
	var indice = GetValor(item, "indice");
	itemli.clave = indice;
	itemli.innerHTML ='<span class="t-h" >' + GetValor(item, "titulo") + '</span>';
	var lista = document.createElement("ul");
	lista.innerHTML = "";	
	items=item.getElementsByTagName("producto");
	var itemli_s;
	for (var n = 0; n < items.length; n++) {
		item=items[n];
		itemli_s = document.createElement("li");
		itemli_s.className="producto";
		itemli_s.setAttribute("onclick","AbrirSelProducto(this);");
		indice = GetValor(item, "clave");
		itemli_s.setAttribute("clave",indice);
		itemli_s.innerHTML ='<img src="https://cdn2.cocinadelirante.com/sites/default/files/styles/gallerie/public/images/2019/09/salsa-para-tortas-ahogada.jpg" style="float: left;max-width: 22%;border-radius: 10px;max-height: 100px;display: block;margin-right: 3%;vertical-align: middle;">'+
		'<span class="t-3 importe" style="float:right;font-weight:bold;margin-top:7px;font-size:0.8em;" importe="'+ GetValor(items[n],"importe") + '">'+ MoneyFormat(GetValor(items[n],"importe")) + '</span><span class="t-1 concepto" style="float: left;width:50%;clear:none;font-weight:bold;font-size:0.85em;">'+ GetValor(items[n],"concepto") + '</span><span class="t-2" style="width:60%;clear:none;font-size:0.7em;">'+ GetValor(items[n],"descripcion") + '</span><hr class="clearn">';
		lista.appendChild(itemli_s);				
	}	
	itemli.appendChild(lista);
	return itemli;
}

function AbrirSelProducto(obj){
	var vent_sel=document.getElementById("desc-prod");
	vent_sel.sel=obj;					
	vent_sel.style.display="block";
	var in_ctd=document.getElementById('ctd');
}

function RealizarConteoCuenta(){
	var l_pedido=document.getElementById("l-pedido");
	var conteo=0;
    var conteos=$(l_pedido).find("span.conteo");
	conteos.each(function() {
        conteo += parseInt(this.getAttribute("conteo"),10);
    });	
	$("#cuenta-car").html(conteo);
	var importe=0.00;
	$(l_pedido).find("span.ctd_imp").each(function() {
        importe += parseFloat(this.getAttribute("ctd_imp"));
    });
	$("#total-c").html(MoneyFormat(importe));
}

function ObtenerProdServ(cat_,item){
	var itemli = document.createElement("li");
	itemli.className = "item";
	var html = "";
	itemli.indice = GetValor(item, "clave");
	itemli.onclick = function () { 
		var clave=this.indice;
		document.getElementById("desc-prod").parentIndice=clave;
		document.getElementById("l-pedido").setAttribute("negocio",clave);
		document.getElementById("l-pedido").innerHTML=window.localStorage.getItem('neg_' + clave);
		RealizarConteoCuenta();
		var nombre=this.nombre;
		var domicilio=this.domicilio;
		$(".h-neg").each(function(){this.innerHTML=nombre;});
		$(".h-ubic-neg").each(function(){this.innerHTML=domicilio;});
		BuscarProductos();
	}
	itemli.nombre=GetValor(item, "NombreNegocio");
	itemli.domicilio=GetValor(item, "domicilio");
	itemli.innerHTML =
		'<div style="width:75%;float:left;"><span class="t-1">' + GetValor(item, "NombreNegocio") + '</span>' +
		'<span class="t-3" style="width:90%;">Horario: ' + GetValor(item, "horario") + '</span>'+
		'<span class="t-2" style="display:inline;width:auto;"><img src="img/call.png" style="float:left;width:1.5em;margin-right:5px;"/> ' + GetValor(item, "telefonos") + '</span>' +
		'<span class="t-3" style="clear:left;"><img src="img/dist.png" style="width:2em;"/> 1.2 KM</span>'+
		'<span class="t-3" style="float:right;width:15%;font-weight:bold;padding:0px;">...</span>'+
		'<span class="t-3" style="float:right;width:30%;padding:0px;"><b style="float:left;margin: 5px 3px;font-weight: bold;font-size: 1.1em;">544</b><img src="img/like.png" style="width:1.8em;"/></span>'+		
		'</div><div style="overflow:hidden;position:relative;float:right;width:23%;margin-right:2%;background-image:url(img/avatar.jpg);background-size:cover;background-position:center center;background-repeat:no-repeat;min-height:100px;border-radius:7px;"/>'+
		'<div class="estatus-pedido solicitado">Solicitado<br/>15 min</div>'+
		'</div>';
	return itemli;
}

function BuscarProdServ(inp) { 
    CargarCatalogo("prodserv", function(){inp.blur();}, { buscar: inp.value },'prodserv-prev',ObtenerProdServ);
}
        ...
        if (_es_admin_) {
            if (this.conceptospagar.length > 0 || this.mesespagar.length > 0) {
                redimAp.innerHTML = "<li class='resumen'><fieldset style='border:1px solid #999;border-radius:10px;margin-bottom:12px;text-align:center;font-weight:bold;font-size:0.9em;'><div style='background:transparent;padding:0px;margin:0px;'><label>Tipo de pago:</label><select id='s_tipo_p' style='width:60%;margin-bottom:5px;margin-right:10px;' name='tipopago' onchange='ResolverUIFormaPago(this);'><option value='11'>EFECTIVO</option><option value='10'>DEPOSITO</option></select><span>Compuesto</span><input type='checkbox' name='es_compuesto' id='es_compuesto' value='true'/>" +
                    ...
                    "<fieldset class='gafet-p' style='border:1px solid #999;border-radius:10px;margin-bottom:12px;text-align:center;font-weight:bold;font-size:0.9em;display:none;'><legend>Gallardetes</legend><div><input id='gafetes-p' type='text' style='width: 90%;padding: 5px;border:1px solid #aaa;text-align:center;font-size:1.1em;' placeholder='#gallardete1:placas,#gallardete2:placas2,...'/></div><div id='tipo_gafete'><span style='margin-left:15px;'>Mensual</span><input checked='checked' type='radio' value='1' name='tipo_gafete' /><span style='margin-left:15px;'>Semestral</span><input type='radio' value='2' name='tipo_gafete' /><span style='margin-left:15px;'>Anual</span><input type='radio' value='3' name='tipo_gafete'/></div></fieldset>" +
                    ...
                    if (document.getElementById("es_compuesto").checked && (num = window.prompt("Ingrese numeración[1,2,..]")).trim().length > 0 || !document.getElementById("es_compuesto").checked) {
                        $.post(url + 'logic/controlador.aspx' + '?op=PagarTicket&seccion=aportaciones' + (this.conceptospagar.length > 0 ? '&conceptos=' + this.conceptospagar.join(",") : '') + (this.mesespagar.length > 0 ? "&meses=" + this.mesespagar.join(",") : '') + "&domicilio=" + document.getElementById("w-datos-persona").getAttribute("domicilio_sel") + (this.fecha ? "&fecha_pago=" + this.fecha : "") + "&tipopago=" + tipo_pago + "&es_compuesto=" + document.getElementById("es_compuesto").checked + "&parte=" + num + "&gafetes=" + gallardetes + "&tipo_g=" + tipoGafetes + "&origen=" + document.getElementById("dep_origen").value, $($(redimAp).find("form")[0]).serializeArray(), function (xmlDoc1) {
                            ...
                            if (GetValor(xmlDoc1, "estatus") == 1) {
                                var precibo=ImprimirReciboCuotas(GetValor(xmlDoc1, "recibo"), GetValor(xmlDoc1, "folio"), GetValor(xmlDoc1, "domicilio"));								
                                CargarAportaciones();
...
function ImprimirReciboCuotas(recibo, folio, domicilio) {
    return window.open("/logic/recibo.pdf?op=GenerarRecibo&seccion=aportaciones&recibo=0&folio=" + folio + "&domicilio=" + domicilio, '_blank', 'width:800,height:600');	
}
...
function IrAlInicio(){
	document.location.href = "#buscar-prodserv";
}
...
function CargarCatalogo(catalogo, callback, parametros, contenedor, callbackin,tag) {
    ...
    $.post(url + 'logic/controlador.aspx' + '?op=' + op + '&seccion=' + catalogo, parametros, function (xmlDoc) {
        var items = xmlDoc.getElementsByTagName(catalogo == "encuestas" ? "Encuesta" : (tag ? tag : "Table"));
        var lista = document.getElementById(contenedor?contenedor:"lista-" + cat_).getElementsByTagName("ul")[0];
        lista.innerHTML = "";
        for (var n = 0; n < items.length; n++) {
			if(callbackin){		
				lista.appendChild(callbackin(cat_, items[n]));
			}else{
...
function VerInforme(clave, config, pdf, xls,fecha1,fecha2) {
    fecha1 = (fecha1!='-1'?fecha1:document.getElementById("i-fecha1").value);
    fecha2 = (fecha2!='-1'?fecha2:document.getElementById("i-fecha2").value);
...
function ObtenerItem(catalogo, item) {
    ...
    switch (catalogo) {
            ...
            itemli.onclick = function (){
                ...
                if (catalogo) {
                    ...
                    $.post(url + 'logic/controlador.aspx' + '?op=ObtenerItem&seccion=' + catalogo + '&claveItem=' + this.indice + (GetValor(item, "proyecto") ? '&proyecto=' + GetValor(item, "proyecto") : "") + (GetValor(item, "actividad") ? '&actividad=' + GetValor(item, "actividad") : "") + "&catalogo=" + catalogo, function (xmlDoc) {
                        //QuitarEspera();                        
                        PintarItem(catalogo, clavel, xmlDoc);
                    });
                }
            }
            itemli.innerHTML =
                '<span class="t-1">' + GetValor(item, "concepto") + '</span>' +
                '<span class="t-2" style="width:50%;">Fecha de pago: ' + GetValor(item, "fecha_pago") + '</span>' +
        ...
        case "ap_domicilios2":
            ...
            var str = '<span class="t-1" >' + GetValor(item, "domicilio") + (GetValor(item, "coincide") == 'T' ? "<i style='color:#0058ff'> : TAG</i>" : "") + (GetValor(item, "coincide") == 'G' ? "<i style='color:#0058ff'> : RECIBO</i>" : "") + '</span>' +
        ...
        case "ap_domicilios":
            ...
            itemli.innerHTML = '<span class="t-1" >' + GetValor(item, "domicilio") + (GetValor(item, "coincide") == 'T' ? "<i style='color:#0058ff'> : TAG</i>" : "") + (GetValor(item, "coincide") == 'G' ? "<i style='color:#0058ff'> : RECIBO</i>" : "") + '</span>' +
                '<span class="t-3">' + GetValor(item, "titular") + '</span>';
            break;
        case "ap_conceptos":
        ...
        case "transparencia":
            ...
            itemli.innerHTML = '<button  class="aceptar" onclick="VerInforme(' + GetValor(item, 'clave') + ',this.parentNode.getAttribute(\'config\'),this.parentNode.getAttribute(\'pdf\'),this.parentNode.getAttribute(\'xls\'),' + '\'' + GetValor(item, 'fecha1') + '\'' + ',' + '\'' +GetValor(item, 'fecha2')+ '\'' + ');">' + GetValor(item, 'descripcion') + '</button>';
            break;
        case "ap_domicilios3":
            var clave = GetValor(item, "clave");
            itemli.innerHTML =
                '<span class="t-1" style="display:inline-block;width:60% !important;">' + GetValor(item, "domicilio") + (GetValor(item, "coincide") == 'T' ? "<i style='color:#0058ff'> : TAG</i>" : "") + (GetValor(item, "coincide") == 'G' ? "<i style='color:#0058ff'> : RECIBO</i>" : "") + '</span><i style="float:right;font-size:0.8em;color:#888;margin-right:15px;">[' + clave + ']</i>' +
        ...
        case "directorio":
            ...
            ; break;
        case "prodserv":
            itemli.onclick = function () { CargarCatalogo('tiposgastos', function () {
					CambioPantalla('lista-tiposgastos', 'lista-prodserv');
				});
			}
            itemli.innerHTML =
...
function IAgregarImagenTexto(id, solotexto, mas, ocultarBtnElim) {
    ...
    if (imagenes.length == 0 || (imagenes.length > 0 && imagenes[imagenes.length - 1].getElementsByTagName("img")[0].getAttribute("sel") == 1) || (imagenes[imagenes.length - 1].getAttribute("cambioTexto") == "true") || mas) {
        ...
        item.className = "lista-files";
        item.innerHTML = '<tbody>' +
            '<tr> <td style="position:relative;width:90%;">' + (solotexto ? '' : '<img src="img/upload.png" onclick="IAdjuntarImagenes(this);" />' + '<input type=file name="vImage" style="position:absolute;left:0px;top:0px;width:100%;height:100%;opacity:0;" onchange="var reader = new FileReader();reader.inp=this;reader.readAsDataURL(this.files[0]);reader.onload=CargarImagenNav;"/>') + '</td> ' + (ocultarBtnElim ? '' : '<td style="width: 10 % " rowspan="2" class="del"><button onclick="QuitarEIT(this' + (solotexto ? ",1" : '') + ');" class="del-btn"><img src="img/del.png" /></button>') + '</td></tr >' +
...
function IAdjuntarImagenes(img, inBtn) {
    ...
    try {
        ...
        window.top.postMessage({ funcion: 'SeleccionarImagenes', datos: [null] }, '*');

    } catch (e) { alert(e.message); }
}

function CargarImagenPhoneGap(result) {
    var img = img_;
    img.contenido = result.split('base64,')[1];
    img.ext = result.split('base64,')[0].split('/')[1].split(';')[0];
    if (new RegExp("pdf", "gi").test(img.ext)) {
        MarcarImagenAdjunta(img, "img/pdf.png", inBtn_);
    } else {
        MarcarImagenAdjunta(img, result, inBtn_);
    }
}
